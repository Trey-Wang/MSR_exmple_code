...
void VttCntrl_Base_UserCallout_OnStateChange(uint8 action, uint8 oldState, uint8 newState) {
  switch(action)
  {
    ...
    {
	  handle_lightOnOffCounter = CANoeAPI_SysVar_DeclareInt("UserDefined::Display", "Display_LightOnOffCounter", 0);
      handle_odometer = CANoeAPI_SysVar_DeclareInt("UserDefined::Display", "Display_Odometer", 0);
      handle_odometerWriteReqPending = CANoeAPI_SysVar_DeclareInt("UserDefined::Display", "Display_OdometerWriteReqPending", 0);
    }
    break;
  }
