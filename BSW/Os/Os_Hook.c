 ...
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL void, OS_CODE, OS_NORETURN, Os_HookReturnWithSchedule, (void));
 ...
 *  \pre            Concurrent access is prevented by caller.
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DECLARATION(OS_LOCAL void, OS_CODE, OS_NORETURN, Os_HookReturnWithoutSchedule, (void));
...
))
{
  ...
  return ((Os_StdReturnType)((CurrentHookStatus->InHookFlags & unallowedNestingFlags) == 0));
}
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_HookReturn, (void))
{
  ...
  switch(currentHook->TaskReturn)
  {
    ...
    /* #40 If returning with schedule call the return function depending on critcal user section. */    
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL void, OS_CODE, OS_NORETURN, Os_HookReturnWithSchedule,
(
  void
))
{
  ...
  if(Os_ThreadIsTask(previousThread) != 0)                                                                              /* SBSW_OS_HK_THREADISTASK_001 */
  {
 ...
 *
 */
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL void, OS_CODE, OS_NORETURN, Os_HookReturnWithoutSchedule, (void))
{
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) currentThread;
  P2CONST(Os_ThreadConfigType, AUTOMATIC, OS_CONST) previousThread;
...
))
{
  ...
  if(Os_HookIsNestingAllowed(activeHooks, Hook) == 0)                                                                   /* SBSW_OS_HK_HOOKISNESTINGALLOWED_001 */
  {
    /* #30 Kernel Panic. */
    Os_ErrKernelPanic();
  }
...
{
  /* #10 If the given hook reference is not NULL: */
  if(Hook != NULL_PTR)
  {
...
)
{
  ...
  /* #10 Get status parameter. */
  status = Os_HookGetDyn(hook)->Parameter.Status;                                                                       /* SBSW_OS_HK_HOOKGETDYN_002 */

  /* #20 Call hook callback function and pass status parameter. */
...
)
{
  ...
  if(Hook != NULL_PTR)
  {
    ...
    if(localCore == targetCore)                                                                                         /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */
    {
...
)
{
  ...
  /* #10 Check that application of given hook is accessible. */
  ...
  {
    /* #20 Use general "hook call" implementation. */
    Os_HookCall(&(Hook->Hook));                                                                                         /* SBSW_OS_FC_PRECONDITION */
 ...
 *
 * */
  if(Os_HookIsPanicHookEnabled() != 0)
  {
    OS_PANICHOOK();
  }
}
...
( void ))
{
  return (Os_StdReturnType)(OS_CFG_PANICHOOK == STD_ON);
}

/***********************************************************************************************************************
 *  Os_Hook2StatusHook()
 **********************************************************************************************************************/
OS_FUNC_ATTRIBUTE_DEFINITION(OS_LOCAL_INLINE P2CONST(Os_HookStatusHookConfigType, AUTOMATIC, OS_CONST), OS_CODE,        /* PRQA S 3219 */ /* MD_Os_3219 */
OS_ALWAYS_INLINE, Os_Hook2StatusHook,
(
  P2CONST(Os_HookConfigType, AUTOMATIC, OS_CONST) Hook
))
{
  return  (P2CONST(Os_HookStatusHookConfigType, AUTOMATIC, OS_CONST))(Hook);                                            /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
{
  return  (P2CONST(Os_HookProtectionHookConfigType, AUTOMATIC, OS_CONST))(Hook);                                        /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
{
  return  (P2CONST(Os_HookInitHookConfigType, AUTOMATIC, OS_CONST))(Hook);                                              /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
{
  return (P2VAR(Os_HookType, AUTOMATIC, OS_VAR_NOINIT))Hook->Thread.Dyn;                                                /* PRQA S 0310 */ /* MD_Os_0310 */
}
...
\CM CM_OS_HOOKID2HOOK_N
      Verify that:
        1. each callback pointer in OsCfg_TaskRefs except the last is no NULL_PTR,
