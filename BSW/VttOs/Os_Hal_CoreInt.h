...
struct Os_Hal_CoreConfigType_Tag
{
  ...
  /*! CANoe Core Id */
  uint32 CANoeEmuCoreId;
};

/*! Platform specific AUTOSAR core configuration. */
struct Os_Hal_CoreAsrConfigType_Tag
{
  /*! Points to the general core configuration data */
  const Os_Hal_CoreConfigType *HalConfig;
  
  /*! Number of CANoe tasks running on this core */
  sint32 NumberOfCANoeEmuTasks;

  /*! Contexts to be initialized by this core */
  const Os_Hal_ContextConfigType **ContextInitList;
};
...
struct Os_Hal_XSigInterruptConfigType_Tag
{
  ...
  /*! Destination core */
  Os_Hal_CoreType      DestinationCore;   /*! \todo The Destination Core is also determined by the rounting mask of this interrupt --> use SetInterrupt() instead of RequestInterruptOnCore() */
};
 ...
 *              It shall not enable memory protection. See also Os_Hal_CoreInit().
 *
