...
))
{
  ...
  return (Os_PriorityQueueNodeIdxType)((2 * NodeIdx) + (uint16_least)Sibling);
}
...
))
{
  ...
  Os_Assert((Os_StdReturnType)(NodeIdx != OS_PRIORITYQUEUE_ROOT_IDX));
...
))
{
  ...
  while(current > OS_PRIORITYQUEUE_ROOT_IDX)
  {
    ...
    if(Os_JobPriorityCompare(PriorityQueue->Queue[current].Data, PriorityQueue->Queue[parent].Data) != 0)               /* SBSW_OS_PQ_JOBPRIORITYCOMPARE_001 */
    {
      P2CONST(void, AUTOMATIC, OS_APPL_DATA) tmpNode;
...
))
{
  ...
  while(left < PriorityQueue->Dyn->QueueUsage)
  {
    ...
    if(Os_JobPriorityCompare(PriorityQueue->Queue[left].Data, PriorityQueue->Queue[right].Data) != 0)                   /* SBSW_OS_PQ_JOBPRIORITYCOMPARE_001 */
    {
...
)
{
  ...
  Os_Assert((Os_StdReturnType)(PriorityQueue->Dyn->QueueUsage < PriorityQueue->QueueSize));
  ...
  (void)Os_PriorityQueueHeapUp(PriorityQueue, (PriorityQueue->Dyn->QueueUsage - 1));                                    /* SBSW_OS_FC_PRECONDITION */
}
...
)
{
  ...
  Os_Assert((Os_StdReturnType)(PriorityQueue->Dyn->QueueUsage > 0));
