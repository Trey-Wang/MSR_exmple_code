...
# SPECIFIC
#
# Specific variables which were defined here are only for local use in this Makefile if for example additional includes
# are necessary, these vars can be defined and used in the REGISTRY-Part.
...
# REGISTRY
#
# $(SSC_PLUGINS) is a list of the components. Here can is added (not necessary for Vector, whole component-list also
# exists in Global.makefile.target.make...)
# e.g.: SSC_PLUGINS += can
SSC_PLUGINS          +=
...
#----------------------------------------------------------------------------------------------------------------------
# MakeSupport usually includes all header-files which were in the same directory as the source-files automatically,
# but to ensure that the Asr-Makefiles will also work with other Basic-Make-Packages, it is necessary to define all
...
# PROVIDE
#
# A Public Makefile does not provide resources for other basic software bundles or the base_make package.
###############################################################################
