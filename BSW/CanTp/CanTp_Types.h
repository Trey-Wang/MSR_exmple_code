...
# define CANTP_TX_QUEUE_SIZE                     4         /* transmit queue size; must be a power of 2              */
#else
...
typedef struct sTxQueueEntrype
{
  ...
  uint8                  SduData[CANTP_MAX_FRAME_LENGTH];       /*!< SduData for CanIf_Transmit */
...
/* for PB-L and PB-S, pointer types are defined by ComStackLib, as they are also needed for the configuration init struct */
#else
typedef P2VAR(CanTp_TxStateType,   AUTOMATIC, CANTP_VAR_NOINIT) CanTp_TxStatePtrType;
typedef P2VAR(CanTp_RxStateType,   AUTOMATIC, CANTP_VAR_NOINIT) CanTp_RxStatePtrType;
#endif
