...
#define CANTP_PCI_LENGTH_LSF                     2
#define CANTP_PCI_LENGTH_LFF                     6

/* common length values */
#define CANTP_CAN20_FRAME_LENGTH                 8         /* CAN 2.0 frame length                                  */
#define CANTP_FF_DL_12BIT                        4095      /* max message length for FF with 12Bit DL               */
#define CANTP_FF_DL_16BIT                        65535     /* max message length for FF with PduLengthType = uint16 */
...
/* frame data handling */
#define CANTP_FRAME_OFFSET_FC_BS                 1         /* block size is stored in FC byte 1          */
#define CANTP_FRAME_OFFSET_FC_STMIN              2         /* separation time is stored in FC byte 2     */
#if (CANTP_CAN20_PADDING_ACTIVE == STD_OFF)
# define CANTP_FRAME_FC_MAX_LENGTH               4         /* TA + FS + BS + STMIN = 4 byte              */
#else
# define CANTP_FRAME_FC_MAX_LENGTH               8         /* TA + FS + BS + STMIN + padding = 8 byte    */
#endif
...
/* support of CanIf_CancelTransmit API */
#if !defined (CANTP_CANIF_ENABLE_TC)
# if defined (CANIF_CANCEL_SUPPORT_API)
#  define CANTP_CANIF_ENABLE_TC                   CANIF_CANCEL_SUPPORT_API     /* Vector CanIf transmit cancellation is available */
# else
#  define CANTP_CANIF_ENABLE_TC                   STD_OFF                      /* CanIf cancellation switch not found, but is optional anyway */
# endif
#endif
...
# define CanTp_RxGetAdrOffset(RxHdl)             ((CanTp_RxCfg_GetAddressingFormat((RxHdl)) == CANTP_ADDRESS_FORMAT_STANDARD) ? ((uint8)0) : ((uint8)1))
#else
# if (CANTP_RX_HAS_STANDARD_ADDRESSING == STD_ON)
   /* only standard addressing is used */
#  define CanTp_RxGetAdrOffset(RxHdl)            (uint8)0
# else
   /* only extended or mixed11 addressing is used */
...
# endif
   ...
   /* multiple addressing formats are allowed; addressing offset can be variable */
# define CanTp_TxGetAdrOffset(TxHdl)             ((CanTp_TxCfg_GetAddressingFormat((TxHdl)) == CANTP_ADDRESS_FORMAT_STANDARD) ? ((uint8)0) : ((uint8)1))
#else
# if (CANTP_TX_HAS_STANDARD_ADDRESSING == STD_ON)
   /* only standard addressing is used */
...
#define CanTp_RxCfg_GetMaxPayloadLSF(RxSduHdl)  ((uint8)(CanTp_RxCfg_GetMaxPayload((RxSduHdl))-1)) /* 1 byte less payload than a CF         */
#define CanTp_RxCfg_GetMaxPayloadFF(RxSduHdl)   ((uint8)(CanTp_RxCfg_GetMaxPayload((RxSduHdl))-1)) /* 1 byte less payload than a CF         */
#define CanTp_RxCfg_GetMaxPayloadLFF(RxSduHdl)  ((uint8)(CanTp_RxCfg_GetMaxPayload((RxSduHdl))-5)) /* 5 byte less payload than a CF         */
...
#define CanTp_TxCfg_GetMaxPayloadLSF(TxHdl)     ((uint8)(CanTp_TxCfg_GetMaxPayload((TxHdl))-1))    /* 1 byte less payload than a CF         */
#define CanTp_TxCfg_GetMaxPayloadFF(TxHdl)      ((uint8)(CanTp_TxCfg_GetMaxPayload((TxHdl))-1))    /* 1 byte less payload than a CF         */
#define CanTp_TxCfg_GetMaxPayloadLFF(TxHdl)     ((uint8)(CanTp_TxCfg_GetMaxPayload((TxHdl))-5))    /* 5 byte less payload than a CF         */
...
# define CanTp_RxCfg_GetMaxSFDataLength(RxHdl, DLC)   (((DLC) >  CANTP_CAN20_FRAME_LENGTH) ? ((DLC) - (CanTp_RxGetAdrOffset((RxHdl)) + CANTP_PCI_LENGTH_LSF)) : (CanTp_RxCfg_GetMaxPayloadSF((RxHdl))))
#else
...
#define CanTp_RxSduSnvIsAvailable(CanTpRxSduId)        CanTp_IsRxSduCfgUsedOfRxSduSnv2Hdl((CanTpRxSduId))              /* PRQA S 3453 */ /* MD_MSR_19.7 */
#define CanTp_TxSduSnvIsAvailable(CanTpTxSduId)        CanTp_IsTxSduCfgUsedOfTxSduSnv2Hdl((CanTpTxSduId))              /* PRQA S 3453 */ /* MD_MSR_19.7 */
/* RxSduConfig <--> TxSduConfig */
#define CanTp_RxSdu2TxSduRefIsAvailable(CanTpRxSduId)  CanTp_IsTxSduCfgUsedOfRxSduCfg((CanTpRxSduId))                  /* PRQA S 3453 */ /* MD_MSR_19.7 */
#define CanTp_TxSdu2RxSduRefIsAvailable(CanTpTxSduId)  CanTp_IsRxSduCfgUsedOfTxSduCfg((CanTpTxSduId))                  /* PRQA S 3453 */ /* MD_MSR_19.7 */

/* RxPduMap --> SduConfig */
#if (CANTP_TX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
# define CanTp_CanIfRxPduHasTxSduRef(CanTpRxPduId)     CanTp_IsTxSduCfgIndUsedOfRxPduMap((CanTpRxPduId))               /* PRQA S 3453 */ /* MD_MSR_19.7 */
#else
...
# define CanTp_CanIfRxPduHasRxSduRef(CanTpRxPduId)     CanTp_IsRxSduCfgIndUsedOfRxPduMap((CanTpRxPduId))               /* PRQA S 3453 */ /* MD_MSR_19.7 */
#else
...
#if (CANTP_RX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
# define CanTp_RxGetSduHdlBySduId(CanTpRxSduId)        (PduIdType)CanTp_GetRxSduCfgIdxOfRxSduSnv2Hdl((CanTpRxSduId))   /* PRQA S 3453 */ /* MD_MSR_19.7 */
#else
# define CanTp_RxGetSduHdlBySduId(CanTpRxSduId)        (PduIdType)0                                                    /* PRQA S 3453 */ /* MD_MSR_19.7 */
#endif
#if (CANTP_TX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
# define CanTp_TxGetSduHdlBySduId(CanTpTxSduId)        (PduIdType)CanTp_GetTxSduCfgIdxOfTxSduSnv2Hdl((CanTpTxSduId))   /* PRQA S 3453 */ /* MD_MSR_19.7 */
#else
# define CanTp_TxGetSduHdlBySduId(CanTpTxSduId)        (PduIdType)0                                                    /* PRQA S 3453 */ /* MD_MSR_19.7 */
#endif
...
#define CANTP_TX_QUEUE_MASK                      (CANTP_TX_QUEUE_SIZE - 1)                                                                          /* bit mask for ring buffer handling */
#define CanTp_TxQueueIsEmpty(Id)                 (CanTp_Data_TxQueue(Id).WritePos == CanTp_Data_TxQueue(Id).ReadPos)                                /* empty: read  = write*/
#define CanTp_TxQueueIsFull(Id)                  (((CanTp_Data_TxQueue(Id).WritePos + 1) & CANTP_TX_QUEUE_MASK) == CanTp_Data_TxQueue(Id).ReadPos)  /* full: (write+1) & mask = read */
...
#  if (CANTP_NUM_RX_SDUS == 0)                                                                                         /* COV_CANTP_ROBUSTNESS_CASE XF */
#   if (CANTP_NUM_RX_CHANNELS != 1)                                                                                    /* COV_CANTP_ROBUSTNESS_CASE XX */
...
#  if (CANTP_NUM_TX_SDUS == 0)                                                                                         /* COV_CANTP_ROBUSTNESS_CASE XF */
#   if (CANTP_NUM_TX_CHANNELS != 1)                                                                                    /* COV_CANTP_ROBUSTNESS_CASE XX */
#    error "Inconsistent setting for number of channels; even if no SDU is configured, the config tool should generate at least one dummy channel"
#   endif
#  else
...
#   endif
#  endif
# endif /* (CANTP_DYN_CHANNEL_ASSIGNMENT == STD_ON) */
